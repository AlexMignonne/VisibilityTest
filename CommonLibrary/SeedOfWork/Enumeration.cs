...
namespace CommonLibrary.SeedOfWork
{
    public abstract class Enumeration : IComparable
    {
        ...
        public static T FromValue<T>(int value)
            where T : Enumeration => GetAll<T>().FirstOrDefault(item => item.Id == value);

        public static bool IsDefined<T>(int value)
            where T : Enumeration => FromValue<T>(value) != null;

        public static T FromDisplayName<T>(string displayName)
            where T : Enumeration => GetAll<T>().FirstOrDefault(item => string.Equals(
                item.Name,
                displayName,
